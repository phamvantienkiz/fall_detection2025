...
def create_windows(df, window_size, features, label_col, stride=10):
    """
    ...
    """
    print(f"\n=== TẠO WINDOWS (window_size={window_size}, stride={stride}) ===")
    ...
    # Tính ngưỡng tối thiểu dựa trên stride
    min_samples_needed = (window_size + stride) * 2  # Ít nhất có thể tạo 3 windows
    ...
    for seq in sequences:
        ...
        if seq_length >= 270:  # Đủ dữ liệu để tạo ít nhất 3 windows (270/90=3)
            ...
            n_windows = seq_length // (window_size + stride)
            ...
            for i in range(0, seq_length - window_size + 1, stride):
                window_start = seq_start + i
                window_end = window_start + window_size
                window_data = df[features].iloc[window_start:window_end].values

                # Đảm bảo shape là (window_size, n_features)
                if window_data.shape != (window_size, len(features)):
                    window_data = window_data.reshape(window_size, len(features))
    ...
    print(f"Shape windows trước khi kiểm tra: {windows.shape}")

    # Fix shape nếu cần (trường hợp shape bị ngược)
    if windows.shape[1] != window_size:
        windows = windows.transpose(0, 2, 1)  # Chuyển từ (n, 3, 90) sang (n, 90, 3)
...
def main():
  ...
  - Đã xử lý: 720 chuỗi
  - Đã bỏ qua: 17 chuỗi
  - Tổng số windows tạo được: 15377
Shape windows trước khi kiểm tra: (15377, 90, 3)
  - Shape của windows: (15377, 90, 3)
  - Shape của labels: (15377,)
  
Tóm tắt:
  - Đã xử lý: 720 chuỗi
  - Đã bỏ qua: 17 chuỗi
  - Tổng số windows tạo được: 17537
Shape windows trước khi kiểm tra: (17537, 60, 3)
  - Shape của windows: (17537, 60, 3)
  - Shape của labels: (17537,)
    """

    # Đổi kiểu dữ liệu các cột features trước khi tạo windows
    ...
    # Tạo windows
    windows, labels = create_windows(df, window_size, features, label_col, stride)

    # Lưu kết quả
    print("\nLưu kết quả...")
    np.save("data_umaf_w90s10.npy", windows)
    np.save("labels_umaf_w90s10.npy", labels)

    print("Đã lưu thành công:")
    print(f"  - data_umaf_w90s10.npy.npy: {windows.shape}")
    print(f"  - labels_umaf_w90s10.npy.npy: {labels.shape}")
