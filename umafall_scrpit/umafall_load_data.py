...
def load_dataset(data_dir="../umafall_dataset", test_size=0.1, val_size=0.1, random_state=42,
                 apply_augment=True, noise_std=0.01, num_augments=2):
    # Load dữ liệu
    X = np.load(os.path.join(data_dir, "data_umaf_w90s10.npy"))  # (samples, 90, 3)
    y = np.load(os.path.join(data_dir, "labels_umaf_w90s10.npy"))
    ...
    joblib.dump(scaler, '../umafall_models/models/scaler_bi_umaf_v4v9.pkl')
