...
from bi_model import build_model, build_model_cnn, build_model_bilstm, build_model_lstm
from load_data import load_dataset

num_classes = 1

def train_model(data_dir, seq_length=90, num_classes=num_classes, epochs=50, batch_size=32):
    ...
    checkpoint = ModelCheckpoint(
        filepath=os.path.join(model_dir, "best_model_bilstm_bi_v1v1.h5"),
    ...
    final_model_path = os.path.join(model_dir, "final_model_bilstm_bi_v1v1.h5")
    model.save(final_model_path)

    # Vẽ biểu đồ Accuracy và Precision theo từng epoch
    ...
    save_result = "../du_md_models/results"
    plt.savefig(os.path.join(save_result, "history_model_bilstm_bi_v1v1.png"))
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_result, "cm_model_bilstm_bi_v1v1.png"))
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_result, "roc_model_bilstm_bi_v1v1.png"))
    plt.show()

# Ví dụ sử dụng
if __name__ == "__main__":
    data_dir = "../du_md_dataset"
    seq_length = 90
    epochs = 50
    batch_size = 32
